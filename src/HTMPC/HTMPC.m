classdef HTMPC < handle
    % Homothetic implementation of CCTMPC. For clarity of presentation, the
    % QP used for computing the closed-loop control law is included in this
    % class instead of solving a single QP. This class acts also as a
    % wrapper for the OCP solver.
    ...
    methods % GETTER methods
        ...
        function y0Sol = get.y0_sol(obj) % y = F*z + \alpha*y_m
            y0Sol = obj.OCP.ccPoly.F*obj.ocpSol{1}(:,1) + ...
                obj.ocpSol{2}(1)*obj.rciSol{1};
        end
    ...
    methods (Access = public)
        function obj = HTMPC(sys, ccPoly, CFM, N, gamma, sol_opts, compileFunc, y_m,u_m)
            if nargin == 7
                [y_m,u_m] = precompute_mRCI_set(ccPoly,CFM.cost_RCI);
            end
            obj.OCP = H_OCP_casadi(sys, ccPoly, CFM, N, gamma, sol_opts, compileFunc, y_m,u_m);
            obj.mpcLaw = obj.initMPCLaw(sol_opts,compileFunc);
            obj.ocpSol = cell(4,1); % {z_t, alpha_t, v_t, costOCP_t} fields
            obj.hausDist = nan;
        end
        ...
        function uMPC = solve(obj, x_curr, varargin)
            ...
            [zOpt,alphaOpt,vOpt,ocpCost] = obj.OCP.solve_OCP(x_curr);
            obj.ocpSol = {full(zOpt),full(alphaOpt),full(vOpt),full(ocpCost)};

            % compute the optimal input (in state space) % z1Opt, alpha1Opt
            uMPC = full( obj.mpcLaw(x_curr, obj.ocpSol{1}(:,2), obj.ocpSol{2}(2)) );
    ...
    methods (Access = private)
        function mpcLaw = initMPCLaw(obj,~,compileFun)
            ...
            F_mat = obj.OCP.ccPoly.F;  % CCPolytope template matrix
            d_vec = obj.OCP.ccPoly.d;  % CCPolytope offset vector
            ...
            z1Opt = opti.parameter(nx,1);
            alpha1Opt = opti.parameter(1,1);

            % For each model, enforce RCT condition
            for i = 1:nm
                opti.subject_to(F_mat*(A_convh{i}*x0 + B_convh{i}*u0Opt) ...
                    + d_vec <= F_mat*z1Opt + alpha1Opt*obj.rciSol{1});
            end
            ...
            mpcLaw = opti.to_function('mpcLaw', {x0, z1Opt, alpha1Opt}, {u0Opt}).expand();
        ...
        % function computeHausdorffDist(obj)
        %     % parallel computation of hausdorff distance
