classdef HTMPC_SinglePolicy < handle
    ...
    methods (Access = public)
        function obj = HTMPC_SinglePolicy(sys, ccPoly, CFM, N, gamma, sdp_opts, OCP_opts)
            ...
            if OCP_opts.useBasicOCP
                obj.OCP = H_SP_OCP_basic(sys, ccPoly, CFM, N, gamma, sdp_opts);
            else
                obj.OCP = H_SP_OCP(sys, ccPoly, CFM, N, gamma, sdp_opts, OCP_opts.var_convh);
            end
    ...
    methods (Access = private)
        ...
        function mpcLaw = initMPCLaw(obj)
            ...
            uVert0Opt = repmat(v0Opt,1,v) + alpha0Opt*obj.OCP.us;

            mpcLaw = optimizer(constr, cost, ...
                obj.OCP.sdp_opts, ...
                {x0, z0Opt, alpha0Opt, v0Opt}, ... % input parameters
                {uVert0Opt*lambda, lambda} ... % output parameters
                ); % (nu x v) * (v x 1) -> (nu x 1)
        end
