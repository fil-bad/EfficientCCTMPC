...
% plot optimal DRTO_set
for j = 1:T_orbit
    han_yper = Polyhedron(ccPoly.F, DRTO_ys{step2plot}(:,j)).plot('alpha',0.5,'Linewidth',0.1,'color',[0.85 0.325 0.098]);%[0.466 0.674 0.188]);
end
pause;

% % plot open-loop CCPolytopes
for j = 1:T_orbit
    han_ys = Polyhedron(ccPoly.F, OCP_ys{step2plot}(:,j)).plot('alpha',0.5,'Linewidth',0.1,'color',[0.466 0.674 0.188]);%[0.85 0.325 0.098]);
end
pause;

for i=1:N_ocp+1
han_y = Polyhedron(ccPoly.F, OCP_y{step2plot}(:,i)).plot('alpha',0.5,'Linewidth',0.1,'color',[0 0.447 0.741]);%[0.929 0.694 0.125]);
end
...
saveas(fig, '../figures/Integrator2D_openLoop.pdf','pdf')