...
B_i = [ 0; 1e-4; 1.3e-3; 5e-2 ];
B = blkdiag(B_i);
nu = size(B,2);

% TOO FAST Dynamics (Ts = 0.05s, it means we have an orbit of 1s with T=20)
% slow them down
Ts_new = 0.35/0.05;

A = (A-eye(nx))*Ts_new + eye(nx);
B = B*Ts_new;

Bw = eye(nx)*Ts_new;    nw = size(Bw,2);
...
% Constraints
e = @(k) double(1:nx==k); % basis vector
% HX = [eye(nx); -eye(nx)]; hX = [0.06;100;pi/4;100; 0.06;100;pi/4;100];
% HX = [e(1); e(3); -e(1); -e(3)]; hX = [0.06; pi/4; 0.06; pi/4];

HX = [e(1); e(2); e(3); e(4);
    -e(1); -e(2); -e(3); -e(4)];
hX = repmat([0.20; 5; pi/6; pi/3], 2,1);


HU = [eye(nu); -eye(nu)]; hU = [10; 10];

HW = [eye(nx); -eye(nx)]; hW = repmat([0; 1e-3; 0; 1e-3],2,1);
...
for f=nx+1:nx+10
F = Polyhedron(numericalSpherePoints(nx, f,4),ones(f,1)).minHRep();
...
F = Polyhedron(numericalSpherePoints(nx, f),ones(f,1)).minHRep();
